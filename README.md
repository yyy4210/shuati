...
| `打开PowerShell.bat` | 双击后在当前文件夹打开 PowerShell（不会命令行的用这个） |
| `部署.bat` | 双击后输入 GitHub 仓库地址，自动完成推送（需先建好仓库） |
...
> 不想自己敲命令？两种偷懒方式：
> 1. 双击 `打开PowerShell.bat`，会自动在当前文件夹打开 PowerShell 窗口。
> 2. 先建好 GitHub 仓库，然后直接双击 `部署.bat`，按提示粘贴仓库地址，它会自动完成推送。
